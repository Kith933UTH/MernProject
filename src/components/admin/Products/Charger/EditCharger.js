...
import { DetailChargerInfo } from './DetailChargerInfo';
...
const DetailData = ({ data, productId, mutate }) => {
	...
	const [details, setDetails] = useState(
		data?.details || DetailChargerInfo.details
	);
	...
	const handleSubmitUpdateDetail = (e) => {
		...
		if (!data) {
			postData(
				'products/chargers/' + productId + '/details',
				{
					guaranteePeriod: 10,
					includedAccessories: [
						'Sách hướng dẫn',
			...
			updateData(
				'products/chargers/' + productId + '/details/' + data._id,
				{
					guaranteePeriod: 10,
					includedAccessories: [
						'Sách hướng dẫn',
	...
	return (
		<div className="w-full bg-white rounded-md border-[1px] border-solid border-gray-300 shadow-sm shadow-gray-400">
			<form>
				<div className="mb-4">
					<Typography className="p-4 text-base font-bold">
						Details
					</Typography>
					<div className="grid grid-cols-2">
						<DetailItem list={details} handleChange={setDetails} />
					</div>
				</div>
...
const VariantItem = ({ data, productId, mutate }) => {
	...
	const handleSubmitUpdateVariant = (e) => {
		...
		patchFormData(
			'products/chargers/' + productId + '/variants/' + data._id,
			formData,
			{
				headers: {
					Authorization: 'Bearer ' + token,
	...
	const handleDeleteVariant = (e) => {
		...
		deleteData('products/chargers/' + productId + '/variants/' + data._id, {
...
const AddVariant = ({ productId, mutate }) => {
	...
	const handleSubmitAddVariant = (e) => {
		...
		postFormData(
			'products/chargers/' + productId + '/variants/',
			formData,
			{
				headers: {
					Authorization: 'Bearer ' + token,
